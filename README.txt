...
- Click visualization tab, select a subject, select an option from the plot
  type drop down, and click visualize
...
- Signal processing pipeline - LED-state deinterleaving, photobleaching
  correction (biexponential), dF/F, isosbestic motion correction, and
  z-scoring. Single-subject or whole-folder batch processing with configurable
  file naming.
...
- {SubjectID}ComputerTS0.csv or {SubjectID}AnimalPosition0.csv - timestamps
  and optional X/Y position. Optional (enables behavior sync).
- boutframes.xlsx - one worksheet per subject; columns are behaviors, values
  are onset frame numbers. Optional (enables bout-aligned analysis).
...
License
-------
This project is distributed under the terms in LICENCE.pdf.
